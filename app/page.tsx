...
export default function Home() {
  ...
  return <main className={`site-shell ${open ? 'opened' : ''}`}>
    ...
    {!open && <button className="invitation-cover kinetic-cover" onClick={() => void handleOpen()} aria-label="Open invitation">
      <span className="cover-edge"/><span className="energy-aura"/><span className="kolam-orbit orbit-one"/><span className="kolam-orbit orbit-two"/><span className="kolam-orbit orbit-three"/>
      <span className="spark-field" aria-hidden="true">{Array.from({ length: 28 }, (_, i) => <i key={i} style={{ '--s': i } as React.CSSProperties}/>)}</span>
      <span className="cover-copy"><img src="/ganesha-gold.png" alt="Ganesha blessing symbol"/><small>Bless our new beginning</small><h2>Sankar <i>&</i> Karthiha</h2><p>Housewarming invitation</p><em>Tap to open</em></span>
    </button>}
  </main>;
}
